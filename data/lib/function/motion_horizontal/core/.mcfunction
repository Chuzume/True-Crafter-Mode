#> lib:motion_horizontal/core/
#
# Motionに関するメイン処理を行います
#
# @within function lib:motion_horizontal/
...
# ノックバック耐性を計算する
    ...
    execute if data storage lib: Argument{KnockbackResist:1b} if score $KnockbackResist Temporary matches 1.. run function lib:motion_horizontal/core/knockback_resistance

# 適用
    execute unless score $VectorMagnitude Temporary matches 0 run function lib:motion_horizontal/core/apply
