...
# 近接当てたら即座にバックステップ
    execute if entity @s[tag=t.hardPiglin_Crossbow_Melee,nbt={OnGround:1b}] if entity @e[type=#t.hard:piglin_enemy,distance=..2,nbt=!{HurtTime:0s}] run scoreboard players set @s TrueCrafterMode.Tick 39

# 背後チェックしてダッシュ/バックステップ
    execute if entity @s[tag=t.hardPiglin_Crossbow_Melee] if entity @e[type=#t.hard:piglin_enemy,distance=..5,tag=!t.hardException] run scoreboard players add @s TrueCrafterMode.Tick 1
    execute if entity @s[tag=!t.hardPiglin_Crossbow_Ranged,scores={TrueCrafterMode.Tick=40}] run function t.hard:enemy/common/leap_attack
    execute if entity @s[tag=!t.hardPiglin_Crossbow_Ranged,scores={TrueCrafterMode.Tick=40}] facing entity @e[type=#t.hard:piglin_enemy,limit=1,sort=nearest] eyes rotated ~ 0 unless block ^ ^-1 ^-3 #t.hard:no_collision if block ^ ^ ^-1 #t.hard:no_collision run scoreboard players set @s Chuz_Speed -7
    execute if entity @s[tag=!t.hardPiglin_Crossbow_Ranged,scores={TrueCrafterMode.Tick=40}] facing entity @e[type=#t.hard:piglin_enemy,limit=1,sort=nearest] eyes rotated ~ 0 unless block ^ ^ ^-1 #t.hard:no_collision run scoreboard players set @s Chuz_Speed 7
    execute if entity @s[tag=!t.hardPiglin_Crossbow_Ranged,scores={TrueCrafterMode.Tick=40}] facing entity @e[type=#t.hard:piglin_enemy,limit=1,sort=nearest] eyes rotated ~ 0 if block ^ ^-1 ^-3 #t.hard:no_collision run scoreboard players set @s Chuz_Speed 7
