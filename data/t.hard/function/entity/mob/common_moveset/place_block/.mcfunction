...
# 水平方向に設置
    execute if entity @s[scores={TrueCrafterMode.PosCheck.Y=0},tag=ChuzOnGround] positioned ^ ^ ^1 rotated ~ 0 if block ~ ~-1 ~ #t.hard:can_place if block ~ ~-2 ~ #t.hard:can_place positioned ^ ^-1 ^ run function t.hard:entity/mob/common_moveset/place_block/place
