...
#ウサギを変換
execute as @e[type=rabbit,distance=..8,nbt=!{RabbitType:99}] at @s run function t.hard:enemy/witch/rabbit_convert
...
execute if entity @s[scores={T.HardMoveset1=40}] facing entity @p[distance=..5] eyes rotated ~ 0 if block ^ ^ ^-1 #t.hard:no_collision run function t.hard:enemy/common/back_warp
execute if entity @s[scores={T.HardMoveset1=40}] facing entity @p[distance=..5] eyes rotated ~ 0 unless block ^ ^ ^-1 #t.hard:no_collision run function t.hard:enemy/common/foward_warp
