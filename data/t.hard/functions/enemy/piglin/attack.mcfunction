...
#何かと敵対してると本気出す
execute if entity @s[nbt={Brain:{memories:{"minecraft:angry_at":{}}}}] run function t.hard:enemy/piglin/true_power
...
#耐火ポーション飲み終わり
execute if entity @s[tag=T.Hard_Piglin_F.Resist,scores={T.Hard_Moveset2=30}] run function t.hard:enemy/piglin/fire_resist_finish

#体力をスコア化
execute store result score @s T.Hard_Health run data get entity @s Health 1
#スコア化された体力が半分以下かつ、プレイヤーとの距離が開いていれば、一度のみ回復する
execute if entity @s[scores={T.Hard_Health=..10},tag=!T.Hard_NoHeal,tag=!T.Hard_Piglin_Heal] unless entity @e[type=#t.hard:piglin_enemy,distance=..8,tag=!T.Hard_Exception] run function t.hard:enemy/piglin/heal
...
effect give @s[tag=T.Hard_Piglin_Heal,scores={T.Hard_Moveset2=20}] instant_health 2 1
#回復完了
execute if entity @s[tag=T.Hard_Piglin_Heal,scores={T.Hard_Moveset2=30}] run function t.hard:enemy/piglin/heal_finish
...
#近接タイプは飛びかかり攻撃してくる
execute if entity @s[tag=T.Hard_Piglin_Sword,tag=!T.Hard_Piglin_F.Resist,tag=!T.Hard_Piglin_Heal] run function t.hard:enemy/piglin/melee
