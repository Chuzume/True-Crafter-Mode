...
# エンドとネザー以外。能動的に追いかけてくるタイプ。
    execute unless dimension minecraft:the_end unless dimension minecraft:the_nether run data modify storage api: Argument.ID set value "enemy.enderman"
...
# オーバーワールド以外、ドラゴンがいない場合
    execute unless dimension minecraft:overworld unless entity @n[type=ender_dragon,distance=..128] run function mob_manager:init/mob/enderman/in_the_end
