...
# 飛びかかり攻撃
    ...
    # 途中で予備動作を行い、スコアが止まらなくなる
        execute if score @s TMCM.Tick matches 30 run function asset:mob/enemy.zombie/in_hostile/windup
    ...
    # 実行
        execute if entity @s[scores={TMCM.Tick=40}] rotated ~ 0 run function asset:mob/enemy.zombie/in_hostile/leap
