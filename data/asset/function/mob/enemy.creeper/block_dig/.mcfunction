...
# 爆発
    execute if score @s 3.Tick.Dig matches 50 at @s run function asset:mob/enemy.creeper/in_hostile/explosion/

# 爆発が終わったあとに無敵を解除し、スコアリセット
    execute if score @s 3.Tick.Dig matches 51.. run function asset:mob/enemy.creeper/in_hostile/explosion/end
