...
# 付近にだれかいるなら起爆準備
    # 20tick以下の場合は、付近に誰かいるならスコア加算
        execute if entity @a[distance=..7] unless score @s General.Mob.Tick matches 20.. run scoreboard players add @s[tag=Standstill] General.Mob.Tick 1
    # ある程度準備すると勝手に加算されるようになる！
        execute if score @s General.Mob.Tick matches 20.. run scoreboard players add @s General.Mob.Tick 1
    # スコア0以上なら起爆演出
...
# 20tick未満の場合は離れられるとスコアリセットする
    execute unless entity @a[distance=..7] if score @s General.Mob.Tick matches 1..19 run attribute @s knockback_resistance base set 0
    execute unless entity @a[distance=..7] if score @s General.Mob.Tick matches 1..19 run scoreboard players remove @s General.Mob.Tick 1
