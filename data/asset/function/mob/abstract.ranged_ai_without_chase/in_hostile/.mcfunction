#> asset:mob/ranged_ai_without_chase/in_hostile/
#
# 付近にいる敵に応じて持ち変える
#
...
# 離れてる時は弓
    execute if entity @s[tag=TMCM.MeleeMode] if entity @n[tag=TMCM.Target,distance=5..16] run function asset:mob/ranged_ai_without_chase/in_hostile/weapon_change/ranged

# 近い時は近接武器
    execute if entity @s[tag=TMCM.RangedMode] if entity @n[tag=TMCM.Target,distance=..5] run function asset:mob/ranged_ai_without_chase/in_hostile/weapon_change/melee
