#> asset:mob/enemy.wither/tick/skill/charge/attack
#
# 
#
# @within function asset:mob/enemy.wither/tick/skill/charge/

# 私が実行者です
    tag @s add This

# 演出
    particle dust_color_transition{from_color:[0.2,0.2,0.2],scale:1.5,to_color:[0.878,0.969,0.576]} ~ ~2 ~ 0.7 1 0.7 1 20 force @a[distance=..48]
...
# 移動
    execute if score @s General.Mob.Tick matches 15..30 on vehicle facing entity @n[tag=TMCM.Target,distance=..48] feet positioned ^ ^ ^-10 rotated as @s positioned ^ ^ ^-80 facing entity @s feet positioned as @s rotated ~ ~ run tp @s ^ ^ ^1 ~ ~
