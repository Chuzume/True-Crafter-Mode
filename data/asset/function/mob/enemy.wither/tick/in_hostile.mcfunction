...
# 通常時の動作
    execute if entity @s[tag=!1025.StopBaseMove,tag=!1025.InDash] run function asset:mob/enemy.wither/tick/base_move
...
# スキル発動、ただし技が一周してない場合
    execute if score @s[tag=!1025.InAction,tag=!1025.CycleReset] General.Mob.Tick matches 60 run function asset:mob/enemy.wither/tick/skill/select/

# 技が一瞬してたら確定で動作を選択
    execute if score @s[tag=!1025.InAction,tag=1025.CycleReset] General.Mob.Tick matches 60.. run function asset:mob/enemy.wither/tick/skill/select/cycle_reset

# スキル発動
    execute if entity @s[tag=1025.InAction] run function asset:mob/enemy.wither/tick/skill/branch

# スキル発動開始時、移動を停止
    execute if score @s[tag=1025.InAction] General.Mob.Tick matches 0 run tag @s add 1025.StopBaseMove

# ダッシュ発動
    execute if entity @s[tag=1025.InDash] run function asset:mob/enemy.wither/tick/dash/branch
