#> asset:mob/enemy.evoker/in_hostile/teleport/recursive_tp
#
# 
#
# @within function 
...
# 移動
    ...
    # 目の前に何も無く、実行座標が自身から離れてなかったらポジションずらして再帰
        execute positioned ^ ^ ^1 if block ~ ~ ~ #true_crafter_mode:no_collision_safe if entity @s[distance=..5] run function asset:mob/enemy.evoker/in_hostile/teleport/recursive_tp
    # 距離限界だったらそこに移動
