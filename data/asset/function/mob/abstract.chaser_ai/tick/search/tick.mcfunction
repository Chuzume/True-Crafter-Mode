...
# ブロック破壊処理
    ...
    # プレイヤーが足元の箱型の判定に収まっている状態が長く続いた場合も、ブロック破壊モードに移行する
        execute unless score @s abstract.chaser_ai.Dig matches 20.. if entity @p[tag=!PlayerShouldInvulnerable,distance=..48] positioned ~-8 ~-34 ~-8 if entity @p[tag=!PlayerShouldInvulnerable,dx=16,dy=32,dz=16] unless block ~ ~-1 ~ #lib:no_dig run scoreboard players add @s abstract.chaser_ai.Dig 1 
    # 条件を満たしていない場合はスコアが減る
        execute if entity @p[tag=!PlayerShouldInvulnerable,distance=..48] anchored eyes positioned ^ ^ ^ facing entity @p[distance=..32] eyes if block ^ ^ ^1 #lib:no_dig at @s positioned ~-8 ~-34 ~-8 unless entity @p[tag=!PlayerShouldInvulnerable,dx=16,dy=32,dz=16] run scoreboard players reset @s abstract.chaser_ai.Dig
...
# やること終わったら解除
    ...
    execute unless entity @p[tag=!PlayerShouldInvulnerable,distance=..48] run function asset:mob/abstract.chaser_ai/tick/search/end
