...
# 敵対していない状態のとき、範囲内にプレイヤーがいたら索敵AIを変更
    execute if entity @p[tag=!PlayerShouldInvulnerable,distance=..64] unless entity @s[tag=3.UsingSeachAI] unless predicate true_crafter_mode:in_hostile run function asset:mob/abstract.chaser_ai/tick/search/start
