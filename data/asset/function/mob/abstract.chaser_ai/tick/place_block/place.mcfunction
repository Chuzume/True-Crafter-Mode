...
# 差がマイナス、つまり自身の座標が対象より低く、かつ自分が止まっているなら
    execute if score @s abstract.chaser_ai.PosCheckY matches ..-1 unless score @s abstract.chaser_ai.PlaceCoolTime matches 0.. if block ~ ~ ~ #lib:can_place anchored eyes rotated ~ 0 if block ^ ^1 ^ #lib:no_collision_safe run function asset:mob/abstract.chaser_ai/tick/place_block/jump
