...
# 足元に誰か収まってるなら下に掘って、以下の処理をスキップ
    execute if entity @p[tag=!PlayerShouldInvulnerable,distance=..64] positioned ~-8 ~-34 ~-8 if entity @p[tag=!PlayerShouldInvulnerable,dx=16,dy=32,dz=16] at @s positioned ~ ~-1 ~ unless block ~ ~ ~ #lib:no_dig run setblock ~ ~ ~ air destroy
    execute if entity @p[tag=!PlayerShouldInvulnerable,distance=..64] positioned ~-8 ~-34 ~-8 if entity @p[tag=!PlayerShouldInvulnerable,dx=16,dy=32,dz=16] at @s run return fail
