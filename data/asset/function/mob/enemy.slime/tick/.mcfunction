...
# 周囲の同族の数を数えて、限界以上だったら膨張はやめる
    execute store result score @s[type=slime] enemy.slime.Count run execute if entity @e[type=slime,distance=..16]
    execute store result score @s[type=magma_cube] enemy.slime.Count run execute if entity @e[type=magma_cube,distance=..16]
