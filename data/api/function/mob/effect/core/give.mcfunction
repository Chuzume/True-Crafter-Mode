...
#
#
# @within function api:mob/effect/give
...
# 継承が行われている場合そのデータを追加する
    execute if data storage asset:effect ID if data storage asset:effect Extends[0] run function api:mob/effect/core/put_id_to_map
# データが正しくあれば呼び出す
