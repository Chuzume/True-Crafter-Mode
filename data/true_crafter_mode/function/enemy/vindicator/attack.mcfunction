...
execute positioned ^ ^ ^-0.5 facing entity @e[type=#true_crafter_mode:human,distance=5..16,limit=1,sort=nearest] feet positioned ^ ^ ^0.5 if entity @s[distance=..0.22] run scoreboard players add @s[nbt={OnGround:1b}] TrueCrafterMode.Tick 1
scoreboard players set @s[scores={TrueCrafterMode.Tick=40}] Chuz.Speed 6
...
#時々バクステで距離を取る
execute if entity @a[distance=..5,tag=!TrueCrafterMode.Exception] run scoreboard players add @s T.HardMoveset2 1
...
execute unless entity @a[distance=..5,tag=!TrueCrafterMode.Exception] run scoreboard players reset @s[scores={T.HardMoveset2=60..}] T.HardMoveset2

#静止していて、なおかつプレイヤーが近ければその方向のブロック破壊
execute if entity @s[nbt={OnGround:1b,Motion:[0.0,-0.0784000015258789d,0.0]}] if entity @a[distance=..20,tag=!TrueCrafterMode.Exception] run scoreboard players add @s TrueCrafterMode.Move.Dig 1
...
execute if entity @s[scores={TrueCrafterMode.Move.Dig=40},tag=ChuzOnGround,tag=ChuzStandstill] facing entity @e[type=#true_crafter_mode:zombie_enemy,distance=..16,sort=nearest,limit=1] eyes rotated ~ 0 run function true_crafter_mode:enemy/common/break_block_forward
