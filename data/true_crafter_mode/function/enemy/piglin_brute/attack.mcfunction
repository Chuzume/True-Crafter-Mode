...
execute unless entity @s[tag=T.HardNoRoar] if entity @e[type=#true_crafter_mode:piglin_enemy,distance=..10,tag=!TrueCrafterMode.Exception] run function true_crafter_mode:enemy/piglin_brute/roar

#自分でも実行する
function true_crafter_mode:enemy/piglin/true_power

#プレイヤーが範囲外なら咆哮、回復の使用制限を解除
execute unless entity @e[type=#true_crafter_mode:piglin_enemy,tag=!TrueCrafterMode.Exception,distance=..25] run tag @s remove T.HardNoRoar
...
execute if entity @s[scores={T.HardHealth=..25},tag=!T.HardNoHeal,tag=!T.HardPiglin_Heal] unless entity @e[type=#true_crafter_mode:piglin_enemy,distance=..8,tag=!TrueCrafterMode.Exception] run function true_crafter_mode:enemy/piglin/heal
...
#時々バクステで距離を取る
execute if entity @e[type=#true_crafter_mode:piglin_enemy,distance=..5,tag=!TrueCrafterMode.Exception] run scoreboard players add @s T.HardMoveset3 1
...
execute unless entity @a[distance=..5,tag=!TrueCrafterMode.Exception] run scoreboard players reset @s[scores={T.HardMoveset3=60..}] T.HardMoveset3
