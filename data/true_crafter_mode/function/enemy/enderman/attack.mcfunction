...
#攻撃に反応してエンダーアイ召喚
execute if entity @s[nbt=!{HurtTime:0s},tag=!T.HardCooldown] if entity @a[distance=..6,tag=!TrueCrafterMode.Exception] run playsound minecraft:entity.enderman.scream hostile @a ~ ~ ~ 1.5 1
execute if entity @s[nbt=!{HurtTime:0s},tag=!T.HardCooldown] if entity @a[distance=..6,tag=!TrueCrafterMode.Exception] run particle minecraft:witch ~ ~2 ~ 0 0 0 1 10
execute if entity @s[nbt=!{HurtTime:0s},tag=!T.HardCooldown] if entity @a[distance=..6,tag=!TrueCrafterMode.Exception] run summon minecraft:vex ~ ~1.5 ~ {Silent:1b,DeathTime:19s,Tags:["T.HardEnder_Eye"],CustomName:'{"text":"Ender Eye"}',ArmorItems:[{},{},{},{id:"minecraft:player_head",Count:1b,tag:{ItemEvent:T.HardEnder_Eye,SkullOwner:{Id:[I;797925833,974278225,-1815402419,531687891],Properties:{textures:[{Value:"eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNjU2ZmYwY2M5OTkyM2NmMjllM2QwNDM2MTA4Mjc1YmUzNjRhM2FjY2IyZDM0ZDhjN2Y1MGNhMTNkYTIzNTgxOCJ9fX0="}]}}}}],ArmorDropChances:[0.085F,0.085F,0.085F,1.000F],ActiveEffects:[{Id:14b,Amplifier:0b,Duration:9999999,ShowParticles:0b}]}
execute if entity @s[nbt=!{HurtTime:0s},tag=!T.HardCooldown] if entity @a[distance=..6,tag=!TrueCrafterMode.Exception] run tag @s add T.HardCooldown
#しばらくしたら召喚を再使用
scoreboard players add @s[tag=T.HardCooldown] TrueCrafterMode.Tick 1
tag @s[scores={TrueCrafterMode.Tick=300}] remove T.HardCooldown
scoreboard players reset @s[scores={TrueCrafterMode.Tick=300}] TrueCrafterMode.Tick

#目の前にブロックがあり、長いことプレイヤーが近くにいるならそれを破壊
execute if entity @a[distance=..6,tag=!TrueCrafterMode.Exception] run scoreboard players add @s[tag=T.HardCooldown] T.HardMoveset2 1
