# すいすい
    ...
    execute as @e[tag=T.HardRotFix,limit=1,sort=nearest] run tp @s ~ ~ ~ facing entity @p[distance=..30,tag=!TMCM.Exception] eyes
    data modify entity @s Rotation[0] set from entity @e[tag=T.HardRotFix,limit=1,sort=nearest] Rotation[0]
    execute facing entity @p[distance=..30,tag=!TMCM.Exception] feet run function true_crafter_mode:move
