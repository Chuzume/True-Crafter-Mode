...
# Y座標がプレイヤーのほうが高く、自分が止まっているならブロック設置
    scoreboard players add @s[scores={TrueCrafterMode.BlockPlace=0..}] TrueCrafterMode.BlockPlace 1
    ...
    execute if entity @s[scores={TrueCrafterMode.BlockPlace=0..}] if block ~ ~-1 ~ #true_crafter_mode:can_place positioned ~ ~-1 ~ run function true_crafter_mode:enemy/block/stone/place
    scoreboard players reset @s[scores={TrueCrafterMode.BlockPlace=5..}] TrueCrafterMode.BlockPlace

# 水平方向に設置
    execute if entity @s[scores={TrueCrafterMode.PosCheck.Y=0},tag=ChuzOnGround] positioned ^ ^ ^1 rotated ~ 0 if block ~ ~-1 ~ #true_crafter_mode:can_place if block ~ ~-2 ~ #true_crafter_mode:can_place positioned ^ ^-1 ^ run function true_crafter_mode:enemy/block/stone/place
