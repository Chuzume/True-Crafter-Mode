...
# 静止していてれば上下のブロック破壊
    execute if entity @s[scores={TMCM.Action.Dig=30,TMCM.PosCheck.Y=..-1},tag=ChuzOnGround,tag=ChuzStandstill] if entity @a[distance=..7,tag=!TMCM.Exception] unless block ~ ~2 ~ #true_crafter_mode:no_dig run setblock ~ ~2 ~ air destroy 
    execute if entity @s[scores={TMCM.Action.Dig=30,TMCM.PosCheck.Y=1..},tag=ChuzOnGround,tag=ChuzStandstill] if entity @a[distance=..7,tag=!TMCM.Exception] unless block ~ ~-1 ~ #true_crafter_mode:no_dig run setblock ~ ~-1 ~ air destroy 
...
# 付近にプレイヤーがいなければY座標を消去
    ...
    execute unless entity @a[distance=..16,tag=!TMCM.Exception] run scoreboard players reset @s TMCM.Action.Dig
