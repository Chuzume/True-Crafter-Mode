...
# オプションで封じられておらず、なおかつ近くにウィザースケルトンがいないならブロック設置
    execute if score #t.hard_Gamerule TrueCrafterMode.Gamerule.PlaceBlock matches 1 unless entity @e[type=wither_skeleton,distance=..30] run function t.hard:enemy/common/block_place_nether
...
# 静止していてれば上下のブロック破壊
    execute if entity @s[scores={TrueCrafterMode.Move.Dig=30,TrueCrafterMode.PosCheck.Y=..-1},tag=ChuzOnGround,tag=ChuzStandstill] if entity @a[distance=..7,tag=!T.HardException] unless block ~ ~2 ~ #t.hard:no_dig run setblock ~ ~2 ~ air destroy 
    execute if entity @s[scores={TrueCrafterMode.Move.Dig=30,TrueCrafterMode.PosCheck.Y=1..},tag=ChuzOnGround,tag=ChuzStandstill] if entity @a[distance=..7,tag=!T.HardException] unless block ~ ~-1 ~ #t.hard:no_dig run setblock ~ ~-1 ~ air destroy 
