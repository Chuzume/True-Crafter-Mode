...
# 足を止めている間スコアを加算し、一定に達するとブロック設置。ただし、橋モードのときはやらない
    execute if entity @s[tag=ChuzStandstill,tag=!TMCM.Action.Bridge] unless score @s TMCM.Action.Dig matches 1.. unless score @s TMCM.Action.PlaceStart matches 20.. run scoreboard players add @s TMCM.Action.PlaceStart 1 
