...
# Y座標がプレイヤーのほうが高く、自分が止まっているならブロック設置
    scoreboard players add @s TrueCrafterMode.BlockPlace 1
    execute if entity @s[scores={TrueCrafterMode.PosCheck.Y=..-1},tag=ChuzOnGround,tag=ChuzStandstill] rotated ~ 0 if block ~ ~ ~ #true_crafter_mode:can_place unless block ~ ~2 ~ #true_crafter_mode:no_dig run function true_crafter_mode:entity/mob/common_moveset/place_block/jump
    execute if entity @s[scores={TrueCrafterMode.BlockPlace=0..}] if block ~ ~-1 ~ #true_crafter_mode:can_place positioned ~ ~-1 ~ run function true_crafter_mode:entity/mob/common_moveset/place_block/place
