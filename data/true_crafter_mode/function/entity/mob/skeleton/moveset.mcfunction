...
# 離れてる時は弓
    execute if entity @s[tag=TMCM.Skeleton.Melee] if entity @n[tag=TMCM.Target,distance=5..16] run function true_crafter_mode:entity/mob/skeleton/weapon_change/ranged

# 近い時は近接武器
    execute if entity @s[tag=TMCM.Skeleton.Ranged] if entity @n[tag=TMCM.Target,distance=..5] run function true_crafter_mode:entity/mob/skeleton/weapon_change/melee

# 近接当てたら即座にバックステップ
    execute if entity @s[tag=TMCM.Skeleton.Melee,nbt={OnGround:1b}] if entity @n[tag=TMCM.Target,distance=..2,nbt=!{HurtTime:0s}] run scoreboard players set @s TMCM.Mob.BackStep 39

# 近接モード時、付近に対象がいる間、バックステップ用スコアを加算
    execute if entity @s[tag=TMCM.Skeleton.Melee] if entity @n[tag=TMCM.Target,distance=..5] run scoreboard players add @s TMCM.Mob.BackStep 1
