...
# 爆発回数加算
    scoreboard players add @s TrueCrafterMode.Creeper.Count 1

# 爆発しても誰かいるなら起爆開始サウンド
    execute if entity @a[distance=..6] run playsound minecraft:entity.creeper.primed hostile @a ~ ~ ~ 1.5 1

# これが3回目の爆発だった場合、虚空へと消える
    execute if score @s TrueCrafterMode.Creeper.Count matches 3.. run function true_crafter_mode:enemy/common/go_to_void
