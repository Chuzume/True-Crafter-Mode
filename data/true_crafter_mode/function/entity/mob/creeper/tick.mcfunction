...
# プレイヤーが微妙な距離にいるなら透明化
    execute if entity @s[tag=!ChuzHurtEntity] unless score @s TMCM.Tick matches 0.. if entity @a[distance=4..32,tag=!TMCM.Exception] run function true_crafter_mode:entity/mob/creeper/stealth/on

# 近くにいれば解除
    execute if entity @s[tag=t.hardStealthMove] if entity @a[distance=..4,tag=!TMCM.Exception] run function true_crafter_mode:entity/mob/creeper/stealth/off
