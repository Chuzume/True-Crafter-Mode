...
# ヒット時に消滅
    execute positioned ~-0.5 ~-0.5 ~-0.5 if entity @a[dx=0] run scoreboard players set @s TrueCrafterMode.Tick 50

# 消滅
    execute if entity @s[scores={TrueCrafterMode.Tick=50..}] on passengers run kill @s
    kill @s[scores={TrueCrafterMode.Tick=50..}]
