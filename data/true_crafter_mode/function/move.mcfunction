...
# XYZにchuz:storageのVectorから100倍して取得
    ...
    execute store result score #Y Chuz_Calc run data get storage chuz:storage Vector[1] 10000
...
# それぞれにChuz_Speed掛ける
    ...
    scoreboard players operation #Y Chuz_Calc *= @s Chuz_Speed
...
# chuz:storageのVectorにそれぞれを戻す
    ...
    execute store result storage chuz:storage Vector[1] double -0.0001 run scoreboard players get #Y Chuz_Calc
...
# リセット
    ...
    scoreboard players reset #Y Chuz_Calc
