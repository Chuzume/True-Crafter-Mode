#> true_crafter_mode:api/colide_check/get_first_motion
#
...
# 初期Motion0だったらタグ付与
    ...
    execute if score @s Chuz.FirstMotion.X matches -5..5 run tag @s add Chuz.DisableColide.X

    execute store result score @s Chuz.FirstMotion.Y run data get entity @s Motion[1] 1000
    execute if score @s Chuz.FirstMotion.Y matches 0 run tag @s add Chuz.DisableColideC.Y

    execute store result score @s Chuz.FirstMotion.Z run data get entity @s Motion[2] 1000
    execute if score @s Chuz.FirstMotion.Z matches -5..5 run tag @s add Chuz.DisableColideZ
