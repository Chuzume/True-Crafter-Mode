#> blessing_engine:asset_manager/artifact/data/current/filter
#
#
#
# @within function
#   blessing_engine:asset_manager/artifact/data/current/get
#   blessing_engine:asset_manager/artifact/data/current/filter
...
# 比較
    ...
    execute store success score $nonIDEquals Temporary run data modify storage asset:artifact Temp set from storage asset:artifact HotbarItems[-1].tag.TSB.ID
# 一致してたら追加
...
# 要素が残ってたら再帰
    execute if data storage asset:artifact HotbarItems[0] run function blessing_engine:asset_manager/artifact/data/current/filter
