#> blessing_engine:asset_manager/artifact/cooldown/main_bar/
#
#
#
...
# if (Maxが0では無い && Valueが負数で無い) ならば表示バーを構築する // ここで負数であるのは-15の場合のみ。
    ...
    execute unless score $Max Temporary matches 0 if score $Value Temporary matches 0.. run function blessing_engine:asset_manager/artifact/cooldown/main_bar/construct_message.m with storage asset:artifact MainBar
# リセット
