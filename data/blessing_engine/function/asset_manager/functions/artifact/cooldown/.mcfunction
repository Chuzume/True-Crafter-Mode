#> blessing_engine:asset_manager/artifact/cooldown/
#
#
#
...
# 各スロットのCDをデクリメントする LCDs(normal) -> DecrementedLCDs(inverted)
    function blessing_engine:asset_manager/artifact/cooldown/mini_bar/foreach_decrement
# 各スロットのCDを表示用に正規化 + DecrementedLCDs(inverted) -> ( LCDs(normal), NormalizedLCDs(normal) )
    function blessing_engine:asset_manager/artifact/cooldown/mini_bar/normalize_and_revert_lcds
# 各スロットのCD表示を構築
    ...
    function blessing_engine:asset_manager/artifact/cooldown/mini_bar/construct_message.m with storage asset:artifact MiniBars
# プレイヤーの現在スロット
    function api:data_get/selected_item_slot
# メインハンドを設定する
    function blessing_engine:asset_manager/artifact/cooldown/main_bar/
# 元に戻す
