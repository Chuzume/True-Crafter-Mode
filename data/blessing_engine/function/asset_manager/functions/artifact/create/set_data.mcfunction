#> blessing_engine:asset_manager/artifact/create/set_data
#
# storageから神器を作成します
#
# @input
...
# 名前
    # 残り回数が存在する場合
        execute if data storage asset:artifact RemainingCount run loot replace block 10000 0 10000 container.0 loot blessing_engine:asset_manager/artifact/get_name/has_remain
    # 残り回数が存在しない場合
        execute unless data storage asset:artifact RemainingCount run loot replace block 10000 0 10000 container.0 loot blessing_engine:asset_manager/artifact/get_name/
    data modify storage asset:artifact Item.tag.display.Name set from block 10000 0 10000 Items[0].tag.display.Name

# Lore
    function blessing_engine:asset_manager/artifact/create/set_lore
# カスタムNBT
