...
#
# @within function
#   blessing_engine:asset_manager/artifact/use/item/*
#   blessing_engine:asset_manager/artifact/use/item/has_remain

# CT設定
    function blessing_engine:asset_manager/artifact/use/item/update_local_cooldown/
# データ更新処理
    ...
    # 残り回数が存在する場合
        ...
        execute if data storage asset:artifact TargetItems[-1].tag.TSB.RemainingCount run loot replace block 10000 0 10000 container.0 loot blessing_engine:asset_manager/artifact/get_name/has_remain
        execute if data storage asset:artifact TargetItems[-1].tag.TSB.RemainingCount run data remove storage asset:artifact Item
    # 残り回数が存在しない場合
        execute unless data storage asset:artifact TargetItems[-1].tag.TSB.RemainingCount run loot replace block 10000 0 10000 container.0 loot blessing_engine:asset_manager/artifact/get_name/
    # スロットをコピーしておく
...
# current更新
    ...
    function blessing_engine:asset_manager/artifact/data/current/update/
# リセット
