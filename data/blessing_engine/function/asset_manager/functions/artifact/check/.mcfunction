#> blessing_engine:asset_manager/artifact/check/
#
# 実行可能かどうかのチェックを行う
#
# @input
...
# 対象スロットのデータの取得
    function blessing_engine:asset_manager/artifact/data/current/get
# 事前にtag付与
    tag @s add CanUsed
# 条件を満たしてなかったらtag削除
    # 信仰による制限
        function blessing_engine:asset_manager/artifact/check/check_believe
    ...
    # SpecialCooldownによる制限
        function blessing_engine:asset_manager/artifact/check/check_special_cooldown
        ...
        tag @s[tag=CheckFailed] remove CheckFailed
    # MP必要量による制限
        function blessing_engine:asset_manager/artifact/check/check_mp
        ...
        tag @s[tag=CheckFailed] remove CheckFailed
    # LocalCooldownによる制限
        function blessing_engine:asset_manager/artifact/check/check_local_cooldown/
...
# 条件を満たしてない && 使用回数が存在する && トリガーがitemUse ならば使用回数を減らす
    execute if entity @s[tag=!CanUsed] if data storage asset:artifact TargetItems[0].tag.TSB.RemainingCount if data storage asset:artifact TargetItems[0].tag.TSB{Trigger:"itemUse"} run function blessing_engine:asset_manager/artifact/use/item/has_remain
# リセット
