#> blessing_engine:asset_manager/teleporter/update_activation_state/
#
...
# テレポーターのIDから対象テレポーターのEntityStorageのIDの取得を試みる
    function blessing_engine:asset_manager/teleporter/update_activation_state/try_get_storage_id_from_teleporter
...
# IDから対象テレポーターのマスタデータを取得する
    execute if score $TeleporterExists Temporary matches 1 run function blessing_engine:asset_manager/teleporter/update_activation_state/pop_master_data_from_teleporter_id
# 状態を更新する
