#> blessing_engine:asset_manager/teleporter/tick/star_init/summon_once
#
#
#
...
# 召喚可能なら召喚する
    execute if data storage asset:teleporter {TPStarSummonable: true} anchored eyes positioned ^ ^ ^ as 0-0-0-0-0 run function blessing_engine:asset_manager/teleporter/tick/star_init/summon
# 召喚したならstorage呼び出し
...
# 召喚できてなかったら再帰
    execute if data storage asset:teleporter {TPStarSummonable:false} if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].Temp.Teleporters[0] run function blessing_engine:asset_manager/teleporter/tick/star_init/summon_once
