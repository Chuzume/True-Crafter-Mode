...
#
# @within function
#   blessing_engine:asset_manager/effect/tick
#   blessing_engine:asset_manager/effect/foreach
...
# 各種イベントを呼び出す
    execute if data storage asset:effect TargetEffect{NextEvent:"given"} run function blessing_engine:asset_manager/effect/events/given/
    execute if data storage asset:effect TargetEffect{NextEvent:"re-given"} run function blessing_engine:asset_manager/effect/events/re-given/
    execute unless data storage asset:effect TargetEffect{NextEvent:"given"} unless data storage asset:effect TargetEffect{Duration:-1} unless entity @s[tag=!DeathProcess,tag=!InRespawnEvent] if data storage asset:effect TargetEffect{ProcessOnDied:"keep"} run function blessing_engine:asset_manager/effect/events/tick/
    ...
    execute unless data storage asset:effect TargetEffect{NextEvent:"given"} unless data storage asset:effect TargetEffect{Duration:-1} if entity @s[tag=!DeathProcess,tag=!InRespawnEvent] run function blessing_engine:asset_manager/effect/events/tick/
    execute if data storage asset:effect TargetEffect{Duration:-1} run function blessing_engine:asset_manager/effect/events/remove/
    execute if data storage asset:effect TargetEffect{Duration:0} run function blessing_engine:asset_manager/effect/events/end/
# フィールドとスタックを元に戻す
...
# まだ処理してないエフェクトがあれば再帰
    execute if data storage asset:effect Effects[0] run function blessing_engine:asset_manager/effect/foreach
