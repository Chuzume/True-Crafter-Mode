#> blessing_engine:asset_manager/spawner/tick/
#
# スポナーのtick処理
#
...
# プレイヤーがRequiredPlayerRangeの範囲内に居るかのチェック
    ...
    execute as @p[gamemode=!spectator,distance=..40] run function blessing_engine:asset_manager/spawner/tick/is_near_player
# this削除
    tag @s remove this
# 範囲内に居る場合tick処理を行う
    execute if entity @s[tag=Success] run scoreboard players remove @s SpawnerCooldown 1
    execute if entity @s[tag=Success] if score @s SpawnerCooldown matches ..-1 run function blessing_engine:asset_manager/spawner/spawn/
# リセット
