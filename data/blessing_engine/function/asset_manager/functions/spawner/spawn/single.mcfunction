#> blessing_engine:asset_manager/spawner/spawn/single
#
# 再帰的にMobを($SpawnCount)体召喚します
#
# @within function
#   blessing_engine:asset_manager/spawner/spawn/
#   blessing_engine:asset_manager/spawner/spawn/single
...
# まだ召喚する必要があるなら召喚するMobをランダムに選択する
    execute if score $SpawnCount Temporary matches 1.. run function blessing_engine:asset_manager/spawner/spawn/choose_mob_id/
# まだ召喚する必要があるなら周囲のEntity数を取得する
    execute if score $SpawnCount Temporary matches 1.. run function blessing_engine:asset_manager/spawner/spawn/nearby_entities/get
# まだ召喚する必要があるなら再帰
    execute if score $SpawnCount Temporary matches 1.. if score $NearbyEntities Temporary < $MaxNearbyEntities Temporary run function blessing_engine:asset_manager/spawner/spawn/single
