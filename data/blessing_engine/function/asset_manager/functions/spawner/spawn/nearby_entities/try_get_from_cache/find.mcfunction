#> blessing_engine:asset_manager/spawner/spawn/nearby_entities/try_get_from_cache/find
#
# 再帰的にキャッシュを検索する
#
# @within function
#   blessing_engine:asset_manager/spawner/spawn/nearby_entities/try_get_from_cache/
#   blessing_engine:asset_manager/spawner/spawn/nearby_entities/try_get_from_cache/find
...
# 見つかっていない かつ 要素がまだあるなら再帰
    ...
    execute if score $NearbyEntities Temporary matches -1 if data storage asset:spawner NearbyEntitiesCache[-1] run function blessing_engine:asset_manager/spawner/spawn/nearby_entities/try_get_from_cache/find
